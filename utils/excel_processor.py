...
# --- Funções de Categorização para Cobranças ---
...
def processar_excel_cobrancas(file_path, file_extension, db_name):
    ...
    try:
        ...
        if file_extension == '.xlsx':
            ...
            except Exception: # Tenta com ponto e vírgula se o primeiro falhar
        ...
        if df_cobrancas is None or df_cobrancas.empty:
            return False, "Ficheiro de cobranças vazio ou não foi possível ler os dados."
        ...
        conceptual_map = {
            'pedido': ['pedido'], 'os': ['os'], 'filial': ['filial'], 'placa': ['placa'],
            'transportadora': ['transportadora'], 'conformidade': ['conformidade'], 'status': ['status']
        }
        ...
        if missing_cols:
            msg = f"Colunas faltando em Cobranças: {', '.join(missing_cols)}. Disponíveis: {original_columns}."
        ...
        for _, row in df_final.iterrows():
            ...
            dados = (str(row.get('filial','')).strip(), str(row.get('placa','')).strip(), str(row.get('transportadora','')).strip(),
                     row.get('conformidade_categorizada'), row.get('status_categorizado'), dt_utc)
            try:
                if exists:
                    cursor.execute("UPDATE cobrancas SET filial=?, placa=?, transportadora=?, conformidade=?, status=?, data_importacao=? WHERE pedido=? AND os=?", (*dados, pedido, os_val))
                    atualizados += 1
                else:
                    cursor.execute("INSERT INTO cobrancas (pedido, os, filial, placa, transportadora, conformidade, status, data_importacao) VALUES (?,?,?,?,?,?,?,?)", (pedido, os_val, *dados))
                    novos += 1
    ...
    try:
        ...
        if file_extension == '.xlsx':
            try: df_pendentes = pd.read_excel(file_path, sheet_name='Pendentes', dtype=str, keep_default_na=False, na_filter=False)
                ...
                logger.warning("Planilha 'Pendentes' não encontrada. Tentando primeira."); excel_file = pd.ExcelFile(file_path)
        ...
        for _, row in mapped_df.iterrows():
            ...
            dados = (pedido_ref, str(row.get('fornecedor','N/A')).strip() or 'N/A', str(row.get('filial','N/A')).strip() or 'N/A', 
                     val_f, status_final, datetime.now(pytz.utc).strftime('%Y-%m-%d %H:%M:%S'))
            try:
                cursor.execute("INSERT INTO pendentes (pedido_ref, fornecedor, filial, valor, status, data_importacao) VALUES (?,?,?,?,?,?)", dados)
                adicionados += 1
            except sqlite3.Error as e: logger.error(f"SQL Erro Pendência (Ref:{pedido_ref}): {e}"); ignorados+=1
        conn.commit()
        return True, f"Pendências: {adicionados} importados, {ignorados} ignorados."
    except FileNotFoundError: return False, f"Ficheiro não encontrado: {file_path}"
    except Exception as e:
        logger.error(f"Erro inesperado ao processar pendências: {e}", exc_info=True)
        return False, f"Erro inesperado ao processar pendências: {str(e)}"
    finally:
        if conn: conn.close()
    return False, "Erro desconhecido no processamento de pendências." # Salvaguarda
    ...
    if filtros:
        for key, val in filtros.items():
            if val: # Apenas adiciona filtro se houver valor
                if key in ['pedido', 'os', 'placa', 'filial', 'transportadora']: # Campos com LIKE
                    conditions.append(f"LOWER({key}) LIKE LOWER(?)")
                    params.append(f"%{val}%")
                elif key in ['status', 'conformidade']: # Campos com correspondência exata (já padronizados)
                    conditions.append(f"LOWER({key}) = LOWER(?)")
                    params.append(val)
...
def get_count_pedidos_status_especifico(status_desejado, db_name='polis_database.db'):
    ...
    except sqlite3.Error as e: logger.error(f"Erro SQL contar status '{status_desejado}': {e}"); return 0
    finally:
        if conn: conn.close()
...
def get_placas_status_especifico(status_desejado, db_name='polis_database.db'):
    ...
    try:
        ...
        cursor.execute(query, (status_desejado,)); return [row['placa'] for row in cursor.fetchall()]
    except sqlite3.Error as e: logger.error(f"Erro SQL buscar placas status '{status_desejado}': {e}"); return []
    finally:
        if conn: conn.close()

def get_count_total_pedidos_lancados(db_name='polis_database.db'): # Agora "lançados" significa "Com cobrança"
    return get_count_pedidos_status_especifico("Com cobrança", db_name)

def get_count_pedidos_nao_conforme(db_name='polis_database.db'): # Agora "não conforme" significa conformidade "Verificar"
    conn = None
    try:
        conn = sqlite3.connect(db_name); cursor = conn.cursor()
        ...
        return count if count is not None else 0
    except sqlite3.Error as e: logger.error(f"Erro SQL contar não conforme: {e}"); return 0
    finally:
        if conn: conn.close()
